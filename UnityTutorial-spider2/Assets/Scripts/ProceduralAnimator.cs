...
public class ProceduralAnimator : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        for (int i = 0; i < _nLimbs; ++i)
        {
            // Draw a raycast for each limb.
            Gizmos.color = Color.green; // You can choose a color for the raycast Gizmos.

            Vector3 limbPosition = _limbs[i].IKTarget.position;
            Vector3 rayOrigin = limbPosition + Vector3.up * _raycastRange;
            Vector3 rayDirection = -Vector3.up;

            // Draw the raycast using Gizmos.DrawRay.
            Gizmos.DrawRay(rayOrigin, rayDirection * _raycastRange);
        }
    }
}
